...
5. Optional for social sign-in coverage: set `GOOGLE_CLIENT_ID` and `GOOGLE_CLIENT_SECRET`
6. `npm run seed`
7. Optional migration safety script: `npm run backfill:neighborhoods`
...
- Auth sessions are stored in the primary PostgreSQL database, so local auth flows only require `DATABASE_URL`.
