...
type PlatformUserProfileFormProps = {
  userId: string;
  name: string;
  email: string;
...
export function PlatformUserProfileForm({
  userId,
  name,
  email,
  initialUsername,
...
}: PlatformUserProfileFormProps) {
  const router = useRouter();
  const { addToast } = useToast();
  const t = useTranslations("platform.userEdit");
  ...
  const updateProfile = trpc.users.updateProfileByAdmin.useMutation();
  ...
  return (
    <form
      ...
      data-testid="platform-user-edit-form"
    >
      ...
      <div className="space-y-6 px-5 py-6">
        ...
        <div>
          <label className="space-y-2 text-sm text-stone-700">
            ...
            <input
              ...
              required
            />
            {!isNameValid ? <p className="text-xs text-red-600">{t("nameError")}</p> : null}
          </label>
        </div>
        ...
        <div>
          <label className="space-y-2 text-sm text-stone-700">
            ...
            <input
              className="vh-v3-focus w-full rounded-lg border border-stone-200 bg-white px-3.5 py-2.5 text-sm text-stone-900 placeholder:text-stone-400 transition-colors hover:border-stone-300 focus:border-teal-400 focus:outline-none"
              data-testid="platform-user-edit-username"
              value={username}
              onChange={(event) => setUsername(event.target.value)}
              placeholder={t("usernamePlaceholder")}
              required
            />
            <p className="text-xs text-stone-400">{t("usernameHelp")}</p>
            {!isUsernameValid ? (
              <p className="text-xs text-red-600">{t("usernameError")}</p>
            ) : null}
          </label>
        </div>
        ...
        <div>
          <label className="space-y-2 text-sm text-stone-700">
            ...
            <select
              ...
              data-testid="platform-user-edit-language"
        ...
        <button
          ...
          type="submit"
          disabled={!canSubmit}
          data-testid="platform-user-edit-submit"
        >
