...
export default async function NeighborPollsPage({
  params,
  searchParams,
}: {
  ...
  const t = await getTranslations("dashboard.pollsList");
    ...
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-8 px-6 py-12">
      ...
      <form className="flex flex-wrap gap-3" method="get">
        <input
          ...
          name="q"
          data-testid="dashboard-polls-search"
        ...
        <button
          ...
          type="submit"
          data-testid="dashboard-polls-filter"
        >
          {t("filter")}
        </button>
      </form>
      ...
      <div className="rounded-xl border border-[color:var(--stroke)] bg-[color:var(--surface)] p-6 shadow-sm">
        {polls.length === 0 ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm" data-testid="dashboard-polls-table">
        ...
        <div className="flex items-center gap-3">
          {page > 1 ? (
            <Link
              ...
              href={`/dashboard/${resolvedParams.groupId}/polls${buildQuery({ q: query || undefined, page: String(page - 1) })}`}
            >
              {t("pagination.prev")}
            </Link>
          ) : null}
